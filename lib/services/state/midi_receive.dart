...
final rxMidiStream = StreamProvider<MidiMessagePacket>((ref) async* {
  final Stream<MidiDataReceivedEvent> rxStream =
      MidiCommand().onMidiDataReceived ?? const Stream.empty();

  final int channel = ref.watch(channelUsableProv);

  await for (final MidiDataReceivedEvent packet in rxStream) {
    final int statusByte = packet.message.data[0];
    ...
    yield MidiMessagePacket(type, packet.message.data.skip(1).toList());
  }
});
...
class _RxNoteVelocitiesNotifier extends Notifier<List<int>> {
  ...
  List<int> build() {
    ref.listen(rxMidiStream, (
      AsyncValue<MidiMessagePacket>? _,
      AsyncValue<MidiMessagePacket> next,
    ) {
